...
def _numeric(value: float | int | Decimal) -> Decimal:
    """Postgres reserve_api_spend expects numeric, not float8."""
...
def settle_spend(*, job_id: UUID, actual_cents: float, status: str) -> None:
    try:
        execute(
            "select settle_api_spend(%s, %s, %s)",
            (job_id, _numeric(max(float(actual_cents), 0.0)), status),
        )
    except Exception:
