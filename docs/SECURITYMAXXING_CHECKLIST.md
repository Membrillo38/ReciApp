...
| 3 | Packages updated | `requirements.txt` pinned; bump on CVE (`pip-audit`) | OK (see deps) |
...
| 6 | Login tokens in secure cookies, not localStorage | Dashboard: `HttpOnly` + `Secure` + `SameSite=Strict`. iOS: JWT in Keychain (client) | OK |
...
| 10 | 2FA on account / hosting / DB / domain | Dashboard TOTP required when secret set. Coolify + Postgres + registrar 2FA = **operator** (not code) | Partial |
...
Privacy Policy + Terms URLs in the iOS app / App Store Connect. Account deletion already exists: `DELETE /v1/me`.
